#!/usr/bin/env python3
"""CPU horizon sweep with the SAME protocol as bench_cpu.py: explicitly
cap torch threads to 8 to match the abstract's 25x/70x numbers."""
...
import numpy as np
import torch
torch.set_num_threads(8)    # MATCH bench_cpu.py
...
def bench_oreg():
    ...
    for h in HORIZONS:
        ...
        for _ in range(N_WARMUP):
            with torch.no_grad(): _ = model(s_t, dt)
        sur_t = []
        ...
        print(f"  oreg h={h:3d}  sur={sur_ms:7.2f}ms  solv={solv_ms:8.1f}ms  "
              f"speedup={out[h]['speedup']:.1f}x", flush=True)
    return out
...
def bench_euler():
    ...
    for h in HORIZONS:
        ...
        env_cfg_p = {"grid": [128, 128], "domain": [0., 1., 0., 1.], "gamma": 1.4, "cfl": 0.4}
        domain = tuple(env_cfg_p["domain"]); nx, ny = env_cfg_p["grid"]
        dx = (domain[1] - domain[0]) / nx; dy = (domain[3] - domain[2]) / ny
        solv_t = []
...
def main():
    out = {}
    out["oregonator"] = bench_oreg()
    out["euler"] = bench_euler()
    out["ball3d"] = bench_ball()
    out_path = RESULTS / "cpu_horizon_sweep_clean.json"
